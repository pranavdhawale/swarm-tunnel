...
create_tunnel() {
  ...
  if [ -f "cloudflare.yml" ]; then
      ...
      echo "❌ 'cloudflare.yml' not found! Cannot deploy stack."
  fi
  pause
}
...
list_dns_records() {
    ...
    echo "----------------------------------------------------------------"
    # echo "DEBUG: Searching for target='$target'"
    # echo "DEBUG: Raw DNS Response: $response"
    
    # Filter for CNAMEs content == target
    # Use -r to get raw strings. 
    # Check if we get any matches by counting lines first?
    # Or just print all CNAMEs for debugging if none found?
    
    echo "$response" | jq -r --arg tgt "$target" '.result[] | select(.content == $tgt) | "\(.id) \(.name)"' | while read -r id name; do
        echo "  - $name"
    done
    
    # Check if empty output (re-run logic or capture above)
    local count
...
add_dns_record() {
    ...
    local service="http://nginx-proxy-manager:81"  # Hardcoded for now per user flow, or valid prompt?
    # User flow says "http://nginx-proxy-manager:81" in reference command.
    # Let's prompt but default to it.
